...
const char BYTECOIN_URI_SCHEME_NAME[] = "owndata";
...
WalletApplication::WalletApplication(int& _argc, char** _argv) : QApplication(_argc, _argv), m_lockFile(nullptr),
  ...
  setApplicationName("owndatawallet");
...
bool WalletApplication::init() {
  ...
  m_lockFile = new QLockFile(Settings::instance().getDataDir().absoluteFilePath("owndatawallet.lock"));
  ...
  if (!m_lockFile->tryLock()) {
    WalletLogger::warning(tr("[Application] Owndata wallet already running"));
    if (!paymentUrl.isValid()) {
      QMessageBox::warning(nullptr, QObject::tr("Fail"), "Owndata wallet already running");
    }
...
bool WalletApplication::initCryptoNoteAdapter() {
  for (;;) {
    ...
    case INodeAdapter::INIT_DB_IN_USAGE: {
      ...
      "If you have owndatad with non-default RPC port, you should terminate it and relaunch OwndataWallet\n"
      "or\n"
      "Set the Local deamon required port in OwndataWallet Menu/Preferences/Connection settings."));
      dlg.exec();
      return false;
    }
