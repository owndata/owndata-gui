...
const char DEFAULT_WALLET_FILE_NAME[] = "owndatawallet.wallet";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("owndatawallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("owndatawallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("owndatawallet.desktop");
  ...
  res = autorunSettings.contains("OwndataWallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("OwndatanWallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    QString autorunFilePath = autorunDir.absoluteFilePath("wallet.plist");
    QSettings autorunSettings(autorunFilePath, QSettings::NativeFormat);
    autorunSettings.remove("Program");
    autorunSettings.setValue("Label", "org.owndata.owndatawallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=Owndata Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("OwndataWallet", appPath);
    } else {
      autorunSettings.remove("OwndataWallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\owndata", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:owndata");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\owndata\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\owndata\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("owndatawallet.cfg"));
